...
class GFPGANModel(BaseModel):
    ...
    def get_roi_regions(self, eye_out_size=80, mouth_out_size=120):
        ...
        self.mouths_gt = self.gt[:,:,174:338,:]
        # image = tensor2img(self.gt[0,:,174:338,:],rgb2bgr=True, min_max=(-1, 1))
        ...
        self.mouths = self.output[:,:,174:338,:]
    ...
    def optimize_parameters(self, current_iter):
        ...
        # get roi-align regions 眼嘴部位的真实值和输出值
        if self.use_facial_disc:
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            if self.use_facial_disc:
                ...
                # mouth
                # cv2.imwrite("test.jpg",numpy.array(self.mouths[0].cpu().detach().numpy(),numpy.int8).transpose(1,2,0))
            ...
            if self.use_identity:
                ...
                # 设置损失函数
                # l_identity = 6
                l_identity = torch.tensor(6).type_as(loss_dict[f'l_p_8']) * identity_weight

                # 展示训练过程中的输入和输出图片
                if current_iter%1000==0:
                    input_show = tensor2img(self.lq[0].squeeze(0),rgb2bgr=True, min_max=(-1, 1))
                    output_show = tensor2img(self.output[0].squeeze(0),rgb2bgr=True, min_max=(-1, 1))
                    print(HyperLPR_plate_recognition(input_show, charSelectionDeskew=False))
                    print(HyperLPR_plate_recognition(output_show, charSelectionDeskew=False))
                    cv2.imwrite("./train_process_show/{}_input.jpg".format(current_iter),input_show)
                    cv2.imwrite("./train_process_show/{}_output.jpg".format(current_iter),output_show)

                for loc,img_path in enumerate(self.gt_path):
                    # image = cv2.imread(img_path)
                    image = tensor2img(self.output[loc].squeeze(0),rgb2bgr=True, min_max=(-1, 1))
                        ...
                        for loc,character in enumerate(result[0][0][1:7]):
                            ...
                            if not '\u4e00' <= character <= '\u9fff':
                                if plate_dict[character.lower()] == real_characters[loc]:
                                    l_identity = l_identity-(1/len(self.gt_path))
